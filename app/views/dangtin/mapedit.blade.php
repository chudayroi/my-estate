...
<script>
   ...
   //creat market
  if(lt!=''){
		 marker1 = new L.marker([lt,lg], {draggable:'true'}).bindPopup("<div><button type='button' class='btn btn-danger btn-xs marker-delete-button'>Xóa</button></div>").openPopup();
   ...
   function onMapClick(e) {
      ...
      var lt,lg,point;
       ...
       marker1 = new L.marker(e.latlng, {draggable:'true'}).bindPopup(" <div><button type='button' class='btn btn-danger btn-xs marker-delete-button'>Xóa</button></div>").openPopup();
        marker1.on("popupopen", onPopupOpen);
        marker1.on("mouseover", mouseover);
     	  lt = marker1.getLatLng().lat.toFixed(5);
     	  lg = marker1.getLatLng().lng.toFixed(5);
